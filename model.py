...
class MAML(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        :return:
        """
        #x_spt, x_qry = x_spt.float(), x_qry.float()
        #print(x_spt.dtype)
        ...
        error = [0 for _ in range(self.update_step + 1)]

        for i in range(task_num):
            # 1. run the i-th task and compute loss for k=0
            logits = self.net(x_spt[i], vars=None, bn_training=True)
            # print('---------------------------------')
            # print(x_spt[i].shape, y_spt[i].shape)
            # print('---------------------------------')           
            #print(logits_q.dtype)
            loss = F.mse_loss(logits, y_spt[i])
            #print(loss.dtype, self.net.parameters())
        ...
        err = np.array(error) / (querysz * task_num)
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry):
        ...
        for k in range(1, self.update_step_test):
            ...
            #with torch.no_grad():
                #pred_q = F.mse_loss(logits_q, dim=1).argmax(dim=1)
        ...
        err = np.array(losses_q) / querysz

        return err, fast_weights
